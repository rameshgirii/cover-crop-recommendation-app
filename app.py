...
def normalize_plant_structure_label(structure):
    s = str(structure).strip().lower()
    if not s:
        return None
    if 'upright' in s:
        return 'Upright'
    if 'basal rosette' in s or 'rosette' in s:
        return 'Basal rosettee'
    if 'creeping' in s or 'prostrate' in s or 'trailing' in s:
        return 'Creeping'
    return None
...
def cn_match_value(avg, pref):
    ...
    if pref == 'Medium':
        return 1.0 if 20 < avg <= 40 else 0.5
    if pref == 'High':
        return 1.0 if avg > 40 else 0.5 if avg > 20 else 0.0
    return 0.5
...
def explain_match(row, prefs):
    ...
    if prefs['preferred_structure'] != 'Any' and normalize_plant_structure_label(row['Plant structure']) == prefs['preferred_structure']:
...
with st.sidebar:
    ...
    structure_options = extract_unique_options(df['Plant structure'], normalize_plant_structure_label)
    preferred_structure = st.selectbox("Preferred plant structure", ['Any'] + structure_options)
...
work['structure_group'] = work['Plant structure'].apply(normalize_plant_structure_label)
work['structure_match'] = (work['structure_group'] == preferred_structure).astype(float) if preferred_structure != 'Any' else 0.5
...
if protein_pref == 'Low':
    work['protein_match'] = cp_vals.apply(lambda x: 1.0 if pd.notna(x) and x < 12 else 0.5 if pd.notna(x) and x < 20 else 0.0)
elif protein_pref == 'Medium':
    work['protein_match'] = cp_vals.apply(lambda x: 1.0 if pd.notna(x) and 12 <= x <= 20 else 0.5 if pd.notna(x) else 0.0)
elif protein_pref == 'High':
    work['protein_match'] = cp_vals.apply(lambda x: 1.0 if pd.notna(x) and x > 20 else 0.5 if pd.notna(x) and x >= 12 else 0.0)
else:
