...
def explain_match(row, prefs):
    ...
    if prefs['preferred_root'] != 'Any' and str(row['Root type']) == prefs['preferred_root']:
...
goal_pool = sorted({item for raw in df['Functional uses'].dropna() for item in split_items(raw)})
...
if protein_pref == 'Low':
    work['cp_norm'] = cp_vals.apply(lambda x: 1.0 if pd.notna(x) and x < 12 else 0.3 if pd.notna(x) and x < 20 else 0.0)
elif protein_pref == 'Medium':
    work['cp_norm'] = cp_vals.apply(lambda x: 1.0 if pd.notna(x) and 12 <= x <= 20 else 0.3 if pd.notna(x) else 0.0)
elif protein_pref == 'High':
    work['cp_norm'] = cp_vals.apply(lambda x: 1.0 if pd.notna(x) and x > 20 else 0.3 if pd.notna(x) and x >= 12 else 0.0)
else:
    work['cp_norm'] = 0.5
...
work['risk_match'] = (6 - risk_raw) / 5
work['root_struct_match'] = (work['root_match'] + work['structure_match']) / 2

w_goal, w_root, w_cp, w_cn, w_risk = 0.35, 0.15, 0.15, 0.15, 0.20
