...
st.write(
    "This prototype uses all 11 fixed dataset parameters through filters, weighted preferences, "
    "and transparent crop result profiles."
)
...
with st.sidebar:
    ...
    region = st.selectbox(
        "Region of growth",
        ["Any"] + sorted(df["Region of growth"].dropna().unique().tolist())
    )

    life = st.selectbox(
        "Life cycle",
        ["Any"] + sorted(df["Life cycle"].dropna().unique().tolist())
    )

    soil_options = sorted(df["Prohibitive soil"].dropna().unique().tolist())
    avoid_soils = st.multiselect(
        "Avoid crops prohibited by these soil conditions",
        soil_options
    )
    ...
    goals = st.multiselect(
        ...
        goal_pool,
        default=["Nitrogen fixation", "Soil erosion reduction"]
    )

    preferred_root = st.selectbox(
        "Preferred root type",
        ["Any"] + sorted(df["Root type"].dropna().unique().tolist())
    )

    preferred_structure = st.selectbox(
        "Preferred plant structure",
        ["Any"] + sorted(df["Plant structure"].dropna().unique().tolist())
    )
...
work["cp_norm"] = (
    pd.to_numeric(work["Crude protein avg"], errors="coerce")
    .fillna(0)
    .clip(lower=min_cp) - min_cp
)
...
if cn_pref == "Low":
    work["cn_match"] = np.where(
        cn_vals.notna(),
        np.where(cn_vals <= 20, 1.0, np.where(cn_vals <= 35, 0.5, 0.0)),
        0.0
    )
elif cn_pref == "Medium":
    work["cn_match"] = np.where(
        cn_vals.notna(),
        np.where((cn_vals > 20) & (cn_vals <= 40), 1.0, 0.3),
        0.0
    )
elif cn_pref == "High":
    work["cn_match"] = np.where(
        cn_vals.notna(),
        np.where(cn_vals > 40, 1.0, np.where(cn_vals > 20, 0.4, 0.0)),
        0.0
    )
else:
    work["cn_match"] = 0.5
...
if work.empty:
    ...
    st.subheader("Extra scoring details")
    trait_view = work[
        [
            "Common name",
    ...
    trait_view = trait_view.rename(
        columns={
            "Nitrogen fixation score": "Nitrogen fixation score",
            "Erosion control score": "Erosion control score",
            "Weed suppression score": "Weed suppression score",
            "Drought tolerance score": "Drought tolerance score",
            "Compaction reduction score": "Compaction reduction score",
            "Forage value score": "Feed usefulness score",
            "Root depth score": "Root system strength score",
            "Soil adaptability score": "Soil flexibility score",
            "Risk factor score": "Management risk score"
        }
    )

    st.dataframe(trait_view, use_container_width=True, hide_index=True)

    st.caption(
        "All 11 fixed parameters are reflected in the interface and result profiles. "
        "Some fields act as filters, some as weighted preferences, and all remain visible for transparency. "
        "The extra scoring details are helper scores derived from the original crop information."
    )
